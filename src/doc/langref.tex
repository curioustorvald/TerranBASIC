...
Some variables are pre-defined on the language itself and cannot be modified; such variables are called \emph{constants}.\index{constants}\index{NIL (constant)}\index{PI (constant)}\index{TAU (constant)}\index{EULER (constant)}
