...
21 & \basiccurry & Left \\
22 & \basicseqapp\quad\basicinfixmap & Right \\
23 & \basicmret & Right \\
24 & \basicclosure\enskip\basicmseq\enskip\basicmbind & Right \\
25 & \basicasgn & Right \\
%\ & \ & \ \\
\end{tabulary}
\end{longtable}
...
\emph{Functor} is just a fancy name for the collection of values that can be \code{MAP}ped.

\begin{tabulary}{\textwidth}{clL}
\index{map (operator)}\index{\basicinfixmap}
\index{sequential application (operator)}\index{\basicseqapp}
Code & Operation & Result \\
\hline
\emph{f} \basicinfixmap\ \emph{xs} & Infix MAP & Executes built-in MAP function on a function \emph{f} and list of values \emph{xs} \\
\emph{fs} \basicseqapp\ \emph{xs} & Sequential Application & MAPs the list of values \emph{xs} for each function in a list of functions \emph{fs}, and concatenates the results \\
\end{tabulary}
