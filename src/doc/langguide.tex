...
But \code{DEFUN F(X)} is only there for partial compatibility with traditional BASICs, of which their syntax is \code{DEF FNX(X)}. \code{DEFUN} cannot define nested functions, it's not a lambda-calculus system, yaddi yadda.
...
Here, \code{[N]$\sim\!>$\,\ldots} defines a \emph{closure} (anonymous function) that has single parameter \code{N}, then assigns it to global variable \code{FAC}.
...
Here, the global function \code{F} has an internal function for a first parameter of \code{MAP}, which is: \code{[X]$\sim\!>$[X]<=5}

Even if the \code{F} and the anonymous function for \code{MAP} uses same parameter name of \code{X}, they're unrelated because two functions are different.
...
But, what if the function was served with pre-cooked curry? The \code{$\sim\!<$} operator is there to curry an un-curried functions; we don't need that if the function was curried in the first place.
...
The function \code{F}, when called, returns its inner function \code{[T]$\sim\!>$ABS(T)==K} with \code{K} being substituted with the argument that were applied to \code{F}, so the function \code{CF} here can be expressed as: \code{[T]$\sim\!>$ABS(T)==32}
