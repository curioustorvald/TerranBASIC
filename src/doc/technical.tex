...
In 1972, dutch mathematician Nicolaas de Bruijn invented \emph{de Bruijn Indexing}, a system we use to solve the forementioned problems.
...
As you can clearly observe, there are two \code{C}s, both must refer to the same bound variable but are in different depth. In order to satisfy the constraint, two \code{C}s have different indices. If you were to change \code{[M]} into \code{[C]}, however, the inner \code{C} will get index of [0,0], which points to the immediate \code{C} and it's different \code{C} than one referred in the outer \code{ZIP}, even if the ZIP's \code{C} has identical index of [0,0].
